<div id="post-{{ $post->id }}" class="post-container w-full rounded-xl bg-white mb-6 p-5 flex gap-5 hover:shadow-card hover:cursor-pointer sm:flex-col sm:gap-3">
    ...
    <div class="flex gap-3 justify-start shrink-0 sm:items-top">
        <img src="https://icon-library.com/images/no-user-image-icon/no-user-image-icon-27.jpg" alt=""
            class="block rounded-xl h-14 w-14">

            <a id="post-{{ $post->id }}-link" class="block font-semibold text-xl smMin:hidden" href="{{ route('posts.show', $post) }}">{{ $post->title }}</a>

            
            <div class="smMin:hidden flex-col hidden">
    ...
    <div class="w-full">
        ...
        <div class="mb-8 sm:mb-4">
            <a id="post-{{ $post->id }}-link" class="block font-bold text-lg mb-3 sm:hidden" href="{{ route('posts.show', $post) }}">{{ $post->title }}</a>
            <p class="text-gray-600 line-clamp-3 sm:line-clamp-5 @if(str_word_count($post->description) <= 1) break-all @endif hover:cursor-text">{{ $post->description  }}</p>
        </div>
